...
} = require('./lib/runtime-content.cjs');
const {
  GSD_CODEX_MARKER,
  CODEX_AGENT_SANDBOX,
...
function installCodexConfig(targetDir, agentsSrc) {
  ...
  for (const file of agentEntries) {
    ...
    content = applyRuntimePathTokens(content, {
      pathPrefix: codexPathPrefix,
      homePathPrefix: toHomePrefix(codexPathPrefix),
      dirName: getDirName('codex'),
    });
...
function copyFlattenedCommands(srcDir, destDir, prefix, pathPrefix, runtime) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      content = applyRuntimePathTokens(content, {
        pathPrefix,
        homePathPrefix: toHomePrefix(pathPrefix),
        dirName: getDirName(runtime),
      });
