...
```bash
PHASE_INFO=$(node "__GSD_HOME_PATH__get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "__GSD_HOME_PATH__get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node "__GSD_HOME_PATH__get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
