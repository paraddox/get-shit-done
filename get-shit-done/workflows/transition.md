...
```bash
TRANSITION=$(node "__GSD_HOME_PATH__get-shit-done/bin/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "__GSD_HOME_PATH__get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
```bash
node "__GSD_HOME_PATH__get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false
```
