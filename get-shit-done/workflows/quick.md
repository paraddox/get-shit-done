...
```bash
INIT=$(node "__GSD_HOME_PATH__get-shit-done/bin/gsd-tools.cjs" init quick "$DESCRIPTION")
...
```bash
node "__GSD_HOME_PATH__get-shit-done/bin/gsd-tools.cjs" commit "docs(quick-${next_num}): ${DESCRIPTION}" --files ${file_list}
```
