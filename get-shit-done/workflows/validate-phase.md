...
```bash
AUDITOR_MODEL=$(node "__GSD_HOME_PATH__get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-nyquist-auditor --raw)
NYQUIST_CFG=$(node "__GSD_HOME_PATH__get-shit-done/bin/gsd-tools.cjs" config get workflow.nyquist_validation --raw)
```
...
```
Task(
  prompt="Read __GSD_PATH__agents/gsd-nyquist-auditor.md for instructions.\n\n" +
...
**State B (create):**
1. Read template from `__GSD_PATH__get-shit-done/templates/VALIDATION.md`
...
node "__GSD_HOME_PATH__get-shit-done/bin/gsd-tools.cjs" commit-docs "docs(phase-${PHASE}): add/update validation strategy"
```
