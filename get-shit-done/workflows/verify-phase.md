...
<required_reading>
@__GSD_PATH__get-shit-done/references/verification-patterns.md
@__GSD_PATH__get-shit-done/templates/verification-report.md
</required_reading>
...
```bash
INIT=$(node "__GSD_HOME_PATH__get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE_ARG}")
...
```bash
node "__GSD_HOME_PATH__get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${phase_number}"
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node "__GSD_HOME_PATH__get-shit-done/bin/gsd-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node "__GSD_HOME_PATH__get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node "__GSD_HOME_PATH__get-shit-done/bin/gsd-tools.cjs" verify artifacts "$plan")
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node "__GSD_HOME_PATH__get-shit-done/bin/gsd-tools.cjs" verify key-links "$plan")
...
See __GSD_PATH__get-shit-done/templates/verification-report.md for complete template.
</step>
