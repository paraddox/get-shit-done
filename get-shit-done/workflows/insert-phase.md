...
```bash
INIT=$(node "__GSD_HOME_PATH__get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "__GSD_HOME_PATH__get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
